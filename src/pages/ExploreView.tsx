...
export function ExploreView() {
  ...
  return (
    <div className="flex flex-col w-full pb-12 space-y-4 animate-in fade-in duration-200">
      {/* Header */}
      <div className="bg-gradient-to-r from-sky-500/10 via-teal-500/10 to-emerald-500/10 rounded-3xl p-4 sm:p-5 border border-sky-100 flex items-center justify-between">
        <div>
          <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-sky-100 text-[10px] font-bold text-sky-900 shadow-2xs mb-1">
        ...
        {/* List / Map Toggle */}
        <div className="flex items-center p-1 bg-white rounded-2xl border border-slate-200 shadow-2xs shrink-0">
          <button
            type="button"
        ...
        <div className="relative bg-slate-100 rounded-3xl overflow-hidden border border-slate-200 shadow-inner h-96 flex flex-col items-center justify-center p-4 text-center">
          ...
          {filteredPlaces.map((place, idx) => {
            ...
            return (
              ...
              >
                <div className="px-2 py-1 rounded-xl bg-slate-900 text-white text-[10px] font-bold shadow-md whitespace-nowrap mb-1 opacity-90 group-hover:scale-105 transition">
                  {place.name.split(' ')[0]}
                </div>
                <div className="w-8 h-8 rounded-full bg-[#ff6b4a] text-white flex items-center justify-center shadow-lg ring-4 ring-white animate-bounce">
          ...
          <div className="relative z-10 bg-white/90 backdrop-blur-md p-3 rounded-2xl shadow-md border border-slate-200 text-xs font-semibold text-slate-700 max-w-xs">
        ...
        <div className="space-y-3">
          {filteredPlaces.length === 0 ? (
            ...
            filteredPlaces.map((place) => (
              <div
                key={place.id}
                className="bg-white rounded-3xl p-4 border border-slate-100 shadow-xs hover:border-orange-100 transition space-y-3"
              >
