  ...
  return (
    <div className="flex flex-col w-full pb-8 space-y-5 animate-in fade-in duration-200">
      ...
      <section className="flex flex-col pt-1 px-1">
        <div className="inline-flex items-center gap-1.5 self-start px-3 py-1 rounded-full bg-[var(--primary)]/10 border border-[var(--primary)]/20 text-[var(--primary)] mb-2 shadow-xs">
          <Sparkles className="w-3.5 h-3.5 fill-[var(--primary)]" />
          <span className="text-xs font-bold tracking-tight">
            Lifetime Connection
          </span>
        </div>
        <h2 className="font-heading font-extrabold text-2xl text-[var(--text)] tracking-tight">
      ...
      <section className="relative overflow-hidden rounded-3xl bg-[var(--card-bg)] p-5 shadow-sm border border-[var(--card-border)]">
        {/* Ambient background glows */}
        <div className="absolute -top-12 -right-12 w-44 h-44 rounded-full bg-[var(--primary)]/10 blur-2xl pointer-events-none" />
        <div className="absolute -bottom-8 -left-8 w-40 h-40 rounded-full bg-[var(--primary)]/5 blur-xl pointer-events-none" />

        <div className="relative z-10 flex flex-col">
          {/* Card Top Pill */}
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <span className="flex items-center justify-center w-7 h-7 rounded-full bg-[var(--primary)] text-white shadow-xs">
          ...
          <div className="flex items-center gap-5 mt-4">
            ...
            <div className="relative flex items-center justify-center shrink-0 w-28 h-28">
              ...
              <div className="absolute flex flex-col items-center justify-center text-center">
                <span className="font-heading text-3xl font-extrabold leading-none text-[var(--primary)]">
                  {pet.level}
                </span>
            ...
            <div className="flex flex-col min-w-0 flex-1">
              ...
              <div className="mt-2 flex items-center gap-1.5 text-amber-800 text-[11px] font-semibold">
                ...
                <span>
                  {Math.max(0, (pet.nextLevelXp || 1000) - (pet.xp ?? 0))} XP to Lvl {(pet.level || 1) + 1}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="mt-5 pt-3.5 bg-slate-50/80 rounded-2xl p-3 border border-slate-100">
            <div className="flex justify-between items-center mb-2">
              ...
              <span className="text-[10px] text-[var(--primary)] font-bold">
                {pet.level >= 30 ? 'Maximum Bond Reached! 👑' : `Next unlock at Lvl ${pet.level < 5 ? '5' : pet.level < 12 ? '12' : pet.level < 20 ? '20' : '30'} 🏷️`}
              </span>
            </div>
            ...
            {(() => {
              ...
              const activeLvl = pet.level || 1;
              // Calculate correct active progress fill width percentage between level 1 and 30
              const progressWidth = Math.min(100, Math.max(0, ((activeLvl - 1) / 29) * 100));
              ...
              return (
                <div className="relative flex items-center justify-between w-full mt-2 px-1">
                  ...
                  {/* Dynamic progress filler line */}
                  <div 
                    style={{ width: `calc(${progressWidth}% - 8px)` }}
                    className="absolute left-4 h-1 bg-[var(--primary)] transition-all duration-1000 z-0" 
                  />

                  {milestonesList.map((m) => {
                    const isUnlocked = activeLvl >= m.lvl;
                    const isActive = activeLvl === m.lvl;

                    return (
                      <div key={m.lvl} className="relative z-10 flex flex-col items-center">
                        {isUnlocked ? (
                          <div className={`w-6.5 h-6.5 rounded-full text-white flex items-center justify-center shadow-xs text-[10px] font-bold transition-all ${
                            isActive 
                              ? 'bg-[var(--primary)] ring-4 ring-[var(--primary)]/20 scale-110 font-black' 
                              : 'bg-[var(--primary)]'
                          }`}>
                        ...
                        <span className={`text-[10px] mt-1 font-semibold ${
                          isActive ? 'text-[var(--primary)] font-black' : isUnlocked ? 'text-slate-700' : 'text-slate-400'
                        }`}>
                          {m.label}
                        </span>
                      </div>
          ...
          {pet.affinityPillars.map((pillar) => (
            ...
            >
              <div className="flex items-center justify-between w-full">
                <div className="w-8 h-8 rounded-full bg-[var(--primary)]/10 border border-[var(--primary)]/20 flex items-center justify-center text-[var(--primary)] font-bold text-sm">
      ...
      <section className="flex flex-col space-y-3">
        <div className="flex items-center justify-between px-1">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-[var(--primary)] hover:opacity-90 text-white text-xs font-bold shadow-xs hover:scale-105 active:scale-95 transition-all"
          >
        ...
        <div className="relative pl-6 space-y-4 before:absolute before:left-2.5 before:top-3 before:bottom-3 before:w-0.5 before:bg-slate-200">
          {memories.map((mem) => (
            ...
            >
              ...
              <div className="absolute -left-6 top-4 w-3.5 h-3.5 rounded-full bg-[var(--primary)] ring-4 ring-[#f7f9fd]" />

              <div className="flex items-center justify-between">
                <span className="text-[10px] text-slate-400 uppercase font-bold tracking-wider">
                  {mem.date}
                </span>
                <span className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-[var(--primary)]/10 text-[var(--primary)] text-[10px] font-bold">
                  +{mem.xp} XP
                </span>
              </div>
      ...
      <section className="flex flex-col space-y-3">
        <div className="flex items-center justify-between px-1">
          ...
          <span className="text-xs text-[var(--primary)] font-bold">6 / 24</span>
        </div>
        ...
        <div className="grid grid-cols-3 gap-2.5">
          {TROPHIES_LIST.map((trophy) => (
            ...
            >
              <div className="w-11 h-11 rounded-full bg-[var(--primary)]/10 border border-[var(--primary)]/20 flex items-center justify-center text-[var(--primary)] shadow-xs mb-1.5 font-bold">
